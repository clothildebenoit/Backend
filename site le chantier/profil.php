...
$contenu .= '<div class="cadre"><h2> Voici tes informations </h2>';
...
<form method="post" action="">
        ...
        <!-- <input type="text" class="form-control" id="pseudo" name="pseudo" maxlength="20" pattern="[a-zA-Z0-9.-_]+" title="caractère autorisés : a-zA-Z0-9.-_"><br> -->
    ...
    if($_POST)
    {
        ...
        {
            $mysqli->query("INSERT INTO commentaire (pseudo, message, date_enregistrement) 
            VALUES ('$_POST[message]', NOW())") OR DIE ($mysqli->error);
            echo '<div class="validation">Votre message a bien été enregistré.</div>';
        }
        else
        {
    ...
    //     echo '</div>';
    // }
    $résultat = $mysqli->query("SELECT '$_POST[pseudo]', message, DATE_FORMAT(date_enregistrement, '%d/%m/%Y') AS datefr, DATE_FORMAT(date_enregistrement, '%H:%i:%s') AS heurefr FROM commentaire ORDER BY date_enregistrement DESC");    
    echo '<h2>'  . $résultat->num_rows . ' commentaire(s)</h2>';
    while($commentaire = $résultat->fetch_assoc())
    {
        echo '<div class="message">';
            echo '<div class="titre">Par: ' . $_SESSION['membre']['pseudo'] . ', le ' . $commentaire['datefr'] . ' à ' . $commentaire['heurefr'] . '</div>';
            echo '<div class="contenu">' . $commentaire['message'] . '</div>';
        echo '</div>';
    }
